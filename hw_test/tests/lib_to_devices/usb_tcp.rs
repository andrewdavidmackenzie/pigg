...
async fn usb_discover_connect_and_disconnect_tcp() {
    ...
    println!(
        "Starting 'usb_discover_connect_and_disconnect_tcp' at {}",
    ...
    println!(
        "Test Ended 'usb_discover_connect_and_disconnect_tcp' at {}",
        end.format("%Y-%m-%d %H:%M:%S")
    );
    println!(
        "Test Duration 'usb_discover_connect_and_disconnect_tcp': {:?}s",
...
async fn usb_discover_connect_and_get_config_tcp() {
    let start: DateTime<Utc> = Utc::now();
    println!(
        "Starting 'usb_discover_connect_and_get_config_tcp' at {}",
        start.format("%Y-%m-%d %H:%M:%S")
    );
    let ip_devices = get_ip_and_port_by_usb()
        .await
    ...
    let end: DateTime<Utc> = Utc::now();
    println!(
        "Test Ended 'usb_discover_connect_and_get_config_tcp' at {}",
        end.format("%Y-%m-%d %H:%M:%S")
    );
    println!(
        "Test Duration 'usb_discover_connect_and_get_config_tcp': {:?}s",
        (end - start).num_seconds()
    );
}
...
async fn usb_discover_connect_and_reconnect_tcp() {
    let start: DateTime<Utc> = Utc::now();
    println!(
        "Starting 'usb_discover_connect_and_reconnect_tcp' at {}",
        start.format("%Y-%m-%d %H:%M:%S")
    );
    let ip_devices = get_ip_and_port_by_usb()
        .await
    ...
    let end: DateTime<Utc> = Utc::now();
    println!(
        "Test Ended 'usb_discover_connect_and_reconnect_tcp' at {}",
        end.format("%Y-%m-%d %H:%M:%S")
    );
    println!(
        "Test Duration 'usb_discover_connect_and_reconnect_tcp': {:?}s",
        (end - start).num_seconds()
    );
}
